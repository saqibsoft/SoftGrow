...
{
    public partial class StockWastage : Form
    {
        bool vOpenMode;
        DAL.StockWastage objDAL = new DAL.StockWastage();
        DAL.Misc Msc = new DAL.Misc();

        private int vUserID;

        public StockWastage(int UserID)
        {
            InitializeComponent();
            vUserID = UserID;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Objects.StockWastage BAL = new Objects.StockWastage();

                BAL.WasteID = Int64.Parse(txtInvNo.Text);
                BAL.EntryDate = dtEntryDate.Value;
                BAL.TotalValue = 0;
                ...
                else
                {
                    ...
                    BAL.WasteID = Int64.Parse(dt.Rows[0]["WasteID"].ToString());
                }
                ...
                foreach (DataGridViewRow dr in Grid.Rows)
                {
                    if (dr.Cells[0].Value != null)
                    {
                        Objects.StockWastageBody objBody = new Objects.StockWastageBody();
                        objBody.WasteID = BAL.WasteID;
        ...
        private void OpenInvoice(Int64 vID)
        {
            try
            {
                ...
                DataTable dt = objDAL.getRecord(" AND  StockWastage.WasteID=" + vID);
                if (dt.Rows.Count > 0)
                {
                    txtInvNo.Text = dt.Rows[0]["WasteID"].ToString();
                    dtEntryDate.Text = dt.Rows[0]["EntryDate"].ToString();                    
        ...
        {
            SearchForms.SchStockWastage vForm = new SearchForms.SchStockWastage();
            vForm.ShowDialog();
        ...
        private void StockWastage_KeyDown(object sender, KeyEventArgs e)
        {
