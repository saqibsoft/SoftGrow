...
namespace SoftGrow.Inventory
{
    ...
    {
        bool vOpenMode;
        DAL.CustomerReturn objDAL = new DAL.CustomerReturn();
        DAL.Misc Msc = new DAL.Misc();

        private int vUserID;

        public CustomerReturn(int UserID)
        {
            InitializeComponent();
            vUserID = UserID;
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Grid.Rows.Add(txt_ProductID.Text, txt_ProductName.Text, 
                    txtUnit.Tag, txtUnit.Text, txt_Qty.Tag, txt_Qty.Text,txtCostPrice.Text);
        ...
        private void OpenInvoice(Int64 vID)
        {
            try
            {
                ...
                DataTable dt = objDAL.getRecord(" AND CustomerReturn.ReturnID=" + vID);
                if (dt.Rows.Count > 0)
                {
                    txtInvNo.Text = dt.Rows[0]["ReturnID"].ToString();
                    dtEntryDate.Text = dt.Rows[0]["ReturnDate"].ToString();
                    txtCustomerID.Text = dt.Rows[0]["CustomerID"].ToString();
                    txtCustomerName.Text = dt.Rows[0]["PartyName"].ToString();                    
                    txtRemarks.Text = dt.Rows[0]["Remarks"].ToString();
                    txtSecurity.Text = decimal.Parse(dt.Rows[0]["SecurityReturn"].ToString()).ToString("G29");
        ...
        {
            SearchForms.SchStockReturn vForm = new SearchForms.SchStockReturn();
            vForm.ShowDialog();
        ...
        private void CustomerReturn_KeyDown(object sender, KeyEventArgs e)
        {
