...
{
    public partial class SchStockWastage : Form
    {
        public SchStockWastage()
        {
            InitializeComponent();
        }
        ...
        {            
                ...
                {
                    MyID = Grid.Rows[Grid.CurrentRow.Index].Cells["WasteID"].Value.ToString();
                }
                Close();
        }
        ...
        private void DisplayData()
        {
            ...
            {
                vWhere = " AND StockWastage.Remarks Like '%" + txtFilter.Text + "%'";
            }

            vWhere += string.Format(" And convert(datetime,Convert(varchar,StockWastage.EntryDate,1)) Between Convert(Datetime,(convert(varchar,convert(Datetime,'{0} 00:00:00',102),1)))and Convert(Datetime,(convert(varchar,convert(Datetime,'{1} 00:00:00',102),1)))", dt_From.Value.ToShortDateString(), dt_ToDate.Value.ToShortDateString());
