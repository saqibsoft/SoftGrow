...
namespace SoftGrow.Finance
{
    public partial class Vouchers : Form
    {
        ...
        public Vouchers(int UserID)
        {
            InitializeComponent();
            vUserID = UserID;
        }

        public void DirectLoad(Int64 VoucherID,string VoucherType)
        {
            ...
            OpenInvoice(VoucherID, VoucherType);
            vDirect = true;
        }
        ...
        private void txtDebit_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (e.KeyChar == '.' && !txtDebit.Text.Contains('.'))
            {
                e.Handled = false; //Do not reject the input
            }
            else
        ...
        private void txtCredit_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else if (e.KeyChar == '.' && !txtCredit.Text.Contains('.'))
            {
                e.Handled = false; //Do not reject the input
            }
            else
        ...
        private void SetMode(bool blnOpen)
        {
            try
            {
                ...
                else
                {
                    ...
                    cboPrintOnVoucher.Text = string.Empty;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please Select or Insert Valid Account #", "Product Missing");
                    txtAccountNo.Focus();
                    return;
                }
                ...
                foreach (DataGridViewRow dr in Grid.Rows)
                {
                    if (dr.Cells[0].Value != null)
                    {
                        ...
                        {
                            MessageBox.Show("Account Already Inserted!!!", "Invalid Entry");
                            txtAccountNo.Focus();
                            return;
                        }
                    }
                ...
                {
                    MessageBox.Show("Zero Value Can not be Inserted.", "Invalid Value");
                    txtNarration.Focus();
                    return;
                }
        ...
        private void OpenInvoice(Int64 vVoucherID, string vVoucherType)
        {
            try
            {
                ...
                {                    
                    ...
                    cboPrintOnVoucher.Text = dt.Rows[0]["PrintVoucherType"].ToString();
                    //Grid.DataSource = dt;
        ...
        private void PrintVoucher(string vVoucherNo, string vVoucherType)
        {
            try
            {                
                string vWhere = string.Empty;                
                ...
                DataTable dt = objDAL.getVouchersData(" AND VoucherHeader.VoucherID=" + vVoucherNo + " AND VoucherHeader.VoucherType='" + vVoucherType + "'");
                if (dt.Rows.Count == 0)
                {
                ...
                else if (vVoucherType == "CPV")
                {
                    ...
                    dr["AccountNo"] = 100000;
                    dr["AccountName"] = "Cash";  //objSet.GetSettingValue(DAL.Settings.ProSettings.CashAccTitle);
                    dr["Credit"] = dt.Rows[0]["Debit"];
                    dr["Debit"] = 0;
                    dr["TranStatus"] = "CREDIT";
