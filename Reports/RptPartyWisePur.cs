...
{
    public partial class RptPartyWisePur : Form
    {
        DAL.Reports objDAL = new DAL.Reports();

        public RptPartyWisePur()
        {
            InitializeComponent();
        }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!chkReturn.Checked)
                {
                    ...
                    dt = objDAL.getPrtyWisePur(vWhere);
                    if (dt.Rows.Count == 0)
                    {
                        MessageBox.Show("No Data To Display", "No Data");
                        return;
                    }
                    vForm.PartyWisePurchase(criteria, dt);
                    vForm.ShowDialog();
                }
                else
                {
                    ...
                    dt = objDAL.getPrtyWisePurRet(vWhere);
                    if (dt.Rows.Count == 0)
                    {
                        MessageBox.Show("No Data To Display", "No Data");
                        return;
                    }
                    vForm.PartyWisePurRet(criteria, dt);
                    vForm.ShowDialog();
                }
