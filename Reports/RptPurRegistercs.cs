...
{
    public partial class RptPurRegistercs : Form
    {
        DAL.Reports objDAL = new DAL.Reports();

        public RptPurRegistercs()
        {
            InitializeComponent();
        }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!chkReturn.Checked)
                {
                    ...
                    {
                        vWhere = " AND Purchase.VendorID=" + this.txtVendorID.Text;
                    }

                    if (!string.IsNullOrEmpty(this.txt_ProductID.Text))
                    {
                        vWhere += " AND PurchaseBody.ProductID=" + this.txt_ProductID.Text;
                    }

                    if (optRange.Checked == true)
                    {
                        vWhere += " AND convert(datetime,Convert(varchar,Purchase.EntryDate,1)) Between Convert(Datetime,(convert(varchar,convert(Datetime,'" + this.dt_From.Text + " 00:00:00',102),1))) AND Convert(Datetime,(convert(varchar,convert(Datetime,'" + this.dt_ToDate.Text + " 00:00:00',102),1)))";
                    ...
                    dt = objDAL.getPurchaseInvoices(vWhere);
                    if (dt.Rows.Count == 0)
                    {
                        MessageBox.Show("No Data To Display", "No Data");
                        return;
                    }
                    vForm.PurchaseRegister(criteria, dt);
                    vForm.ShowDialog();
                }
                else
                {
                    if (!string.IsNullOrEmpty(this.txtVendorID.Text))
                    {
                        vWhere = " AND PurReturn.VendorID=" + this.txtVendorID.Text;
                    }

                    if (!string.IsNullOrEmpty(this.txt_ProductID.Text))
                    {
                        vWhere += " AND PurReturnBody.ProductID=" + this.txt_ProductID.Text;
                    }

                    if (optRange.Checked == true)
                    {
                        vWhere += " AND convert(datetime,Convert(varchar,PurReturn.EntryDate,1)) Between Convert(Datetime,(convert(varchar,convert(Datetime,'" + this.dt_From.Text + " 00:00:00',102),1))) AND Convert(Datetime,(convert(varchar,convert(Datetime,'" + this.dt_ToDate.Text + " 00:00:00',102),1)))";
                        criteria = "From: " + dt_From.Value.ToString("dd-MMM-yyyy") + " To: " + dt_ToDate.Value.ToString("dd-MMM-yyyy");
                    }
                    else
                    {
                    ...
                    dt = objDAL.getPurchaseReturnInvoices(vWhere);
                    if (dt.Rows.Count == 0)
                    {
                        MessageBox.Show("No Data To Display", "No Data");
                        return;
                    }
                    vForm.PurchaseRetRegister(criteria, dt);
                    vForm.ShowDialog();
                }
