...
{
    public partial class RptPartySaleSummary : Form
    {
        DAL.Reports objDAL = new DAL.Reports();
        public RptPartySaleSummary()
        {
            InitializeComponent();
        }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DateTime vToDate = dt_ToDate.Value;
                    ...
                    DataTable dt = objDAL.getPartySaleSummary(txtVendorID.Text, int.Parse(cboSalesman.SelectedValue.ToString()), vFromDate.ToShortDateString(), vToDate.ToShortDateString());
                    if (dt.Rows.Count == 0)
                    {
                        MessageBox.Show("No Data To Display", "No Data");
                        return;
                    }
                    vForm.PartySaleSummary(criteria, dt);
                    vForm.ShowDialog();
